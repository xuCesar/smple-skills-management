...
fn load_config() -> Result<serde_json::Value, String> {
  let path = config_path()?; if !path.exists() { return Ok(serde_json::json!({"version":1,"directories":[{"path":"~/.agents/skills","source":"default"},{"path":"~/.codex/skills","source":"default"},{"path":"~/.claude/skills","source":"default"}],"hiddenSkillIds":[],"installations":[],"updatedAt": chrono_now()})); }
  let raw = fs::read_to_string(&path).map_err(|e| e.to_string())?;
  match serde_json::from_str(&raw) { Ok(value) => Ok(value), Err(_) => { fs::rename(&path, path.with_extension(format!("corrupt.{}", chrono_now()))).map_err(|e| e.to_string())?; Ok(serde_json::json!({"version":1,"directories":[{"path":"~/.agents/skills","source":"default"},{"path":"~/.codex/skills","source":"default"},{"path":"~/.claude/skills","source":"default"}],"hiddenSkillIds":[],"installations":[],"updatedAt":chrono_now()})) } }
}
#[tauri::command]
